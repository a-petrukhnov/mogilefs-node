...
Domain.prototype.create_domain = function( d_name, callback){
	callback = callback || noop;
	d_name = d_name || null;

	var args = null
	console.log(d_name);	
	this.mogile.send(d_name, 'CREATE_DOMAIN', null, function(err,response) {
		if(err){
...
Domain.prototype.delete_domain = function( name,callback){
	callback = callback || noop;
	name = name || null;
	
	this.mogile.send(name, 'DELETE_DOMAIN', null, function(err,response) {
                if(err){
                        return callback(console.log(err));
                }
                callback();
 ...
 * Get domain list 
 *
 * @param {Function} callback return domain list
 * @return {Boolean}
 */

Domain.prototype.get_domain = function(callback){
	callback = callback || noop;
	domain_list = [];
               this.mogile.send(null, 'GET_DOMAINS', null, function(err,response) {
                if(err){
                        return callback(console.log(err));
                }
		for (let f=1 ; f <= response['domains'] ; f++){
			domain_list.push(response['domain'+f]);
		}
                return callback(domain_list);
        })
}
 ...
 * @param {Function} callback create class information
 * @return {Boolean}
 */


Domain.prototype.create_class = function (key , mindevcount , callback){
	callback = callback || noop;
	key = key || noop;
	mindevcount = mindevcount || 2;
	if(key == 'default'){
                return console.log(" Can't create default class");
        }
	var args = {
                'domain': this.name,
                'class' : key,
	...
	this.mogile.send(this.name, 'CREATE_CLASS', args, function(err,response) {
                if(err){
                        return callback(console.log(err));
                }
                return callback();
...
Domain.prototype.update_class = function (key , mindevcount , callback){
        callback = callback || noop;
        key = key || noop;
        mindevcount = mindevcount || 2;
        ...
        this.mogile.send(this.name, 'UPDATE_CLASS', args, function(err,response) {
                if(err){
                        return callback(console.log(err));
                }
                return callback();
 ...
 *
 * @param {String} key  class name
 * @param {Function} callback delete class information
 * @return {Boolean}
 */


Domain.prototype.delete_class = function ( key, callback){
	callback = callback || noop;
	key = key || noop;
	if(key == 'default'){
		return console.log(" Can't delete default class");
	}
	var args = {
                'domain': this.name,
                'class' : key
        };
	this.mogile.send(this.name, 'DELETE_CLASS', args, function(err,response) {
		if(err){
                        return callback(console.log(err));
                }
		return callback();
 ...
 * @param {Function} callback class information
 * @return {Boolean}
 */


Domain.prototype.get_class = function(key_domain,callback){
        callback = callback || noop;
        class_list = [];
               this.mogile.send(null, 'GET_DOMAINS', null, function(err,response) {
                if(err){
                ...
                for (let f=1 ; f <= response['domains'] ; f++){
                        if (key_domain === response['domain'+f]){
				for (let c = 1 ; c <= response['domain'+f+'classes'] ; c++){
	if (response['domain'+f+'class'+c+'name'] !== 'default'){
		class_list.push(response['domain'+f+'class'+c+'name']);
	}
}
				
			}
                }
                return callback(class_list);
        })
}
