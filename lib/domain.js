...
Domain.prototype.createClass = function (key , mindevcount , callback){
    callback = callback || noop;
    key = key || noop;
...
Domain.prototype.updateClass = function (key , mindevcount , callback){
        callback = callback || noop;
        key = key || noop;
        mindevcount = mindevcount || 2;
...
Domain.prototype.deleteClass = function ( key, callback){
    callback = callback || noop;
    key = key || noop;
    if(key == 'default'){
 ...
 * @param {Function} callback FileInfo information
 * @return {Boolean}
 */
Domain.prototype.fileInfo = function ( key, callback){
    callback = callback || noop;
    key = key || noop;
    var args = {
                'domain': this.name,
    ...
    this.mogile.send(this.name, 'file_Info', args, function(err,response) {
        if(err){
                        return callback(console.log(err,response));
                }
